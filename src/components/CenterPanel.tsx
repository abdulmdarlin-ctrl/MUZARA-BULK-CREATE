...
export function CenterPanel() {
  ...
  }, [canvas, interactionMode]); // Listen to mode changes
    ...
    const renderCanvas = async () => {
      ...
      if (templateUrl && templateFile) {
        try {
          ...
          } else if (templateFile.name.toLowerCase().endsWith('.tif') || templateFile.name.toLowerCase().endsWith('.tiff')) {
            // Handle TIFF files
            try {
              const response = await fetch(templateUrl);
              const arrayBuffer = await response.arrayBuffer();
              const ifds = (window as any).UTIF.decode(arrayBuffer);
              (window as any).UTIF.decodeImage(arrayBuffer, ifds[0]);
              const rgba = (window as any).UTIF.toRGBA8(ifds[0]);
              
              const canvasEl = document.createElement('canvas');
              canvasEl.width = ifds[0].width;
              canvasEl.height = ifds[0].height;
              ...
              console.error("Error rendering TIFF:", tifErr);
              throw new Error("Failed to render TIFF template. Ensure UTIF.js is loaded.");
            }
          }
  ...
  useEffect(() => {
    ...
    fields.forEach(field => {
      ...
      if (obj) {
        ...
        if (field.type !== 'image' && (obj as any).textAlign !== field.align) (obj as any).set('textAlign', field.align);
        if (field.type !== 'image' && (obj as any).text !== (field.value || field.label)) (obj as any).set('text', field.value || field.label);
        obj.setCoords();
      } else {
        // ADD NEW FIELD IF IT DOES NOT EXIST YET!
        const { interactionMode } = useStore.getState();
        const isSelectable = interactionMode !== 'place_point';
    ...
    // Also remove any canvas objects that are no longer in the fields array
    const canvasObjects = canvas.getObjects();
    const objectsToRemove: any[] = [];
    canvasObjects.forEach((obj: any) => {
      if (obj.id && !fields.find(f => f.id === obj.id)) {
